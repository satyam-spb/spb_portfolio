...
export const Projects: React.FC = () => {
  ...
  return (
    <section id="projects" className="py-24 w-full max-w-4xl px-6">
      <div className="flex flex-col items-center mb-12 text-center">
        <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50 mb-4">Engineering Showcase</h2>
        ...
        <p className="text-zinc-500 dark:text-zinc-400 mb-8 max-w-lg">
           Production-grade systems demonstrating scale, security, and algorithmic complexity.
        </p>
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              ...
              <div className="hidden sm:flex sm:col-span-2 flex-col items-end pt-1">
                   ...
                   <div className={`w-px h-full min-h-[100px] ${project.isFeatured ? 'bg-indigo-400 dark:bg-indigo-500' : 'bg-zinc-200 dark:bg-zinc-800'} transition-colors duration-300`}></div>
              </div>

              <div className={`sm:col-span-10 flex flex-col h-full border rounded-2xl p-6 sm:p-8 transition-all duration-300 ${
                  project.isFeatured 
                  ...
                  <div className="space-y-4 mb-8 flex-grow">
                      <div className="grid grid-cols-[80px_1fr] gap-2">
                           <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mt-1">Problem</span>
                           <p className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">{project.problem}</p>
                      </div>
                      <div className="grid grid-cols-[80px_1fr] gap-2">
                           <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mt-1">Solution</span>
                           <p className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed">{project.solution}</p>
                      </div>
                      <div className="grid grid-cols-[80px_1fr] gap-2">
                           <span className="text-xs font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wide mt-1">Impact</span>
                  ...
                  <div className="flex flex-wrap gap-2 pt-5 border-t border-zinc-100 dark:border-zinc-800">
                      {project.tech.map((t) => (
                          <span key={t} className="px-2.5 py-1 bg-zinc-50 dark:bg-zinc-800/80 border border-zinc-100 dark:border-zinc-700/50 rounded-md text-[11px] font-mono font-medium text-zinc-500 dark:text-zinc-400">
                              {t}
                          </span>
                      ))}
                  </div>
